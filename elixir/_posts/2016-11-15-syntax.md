...
| U  |  inverts the “greediness” of the regular expression |
...
Functions can be defined with `def/2` and private functions with `defp/2`. Functions are also a type. Arity is the number of parameters of a given function: `Enum.map/2`.
...
Module attributes are variables defined inside modules (not functions) used to annotate the module and as constants. Attributes can be read inside functions. The value exists only during compilation time.
