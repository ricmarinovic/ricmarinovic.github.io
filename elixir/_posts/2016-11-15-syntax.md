...
`Any` can be used to match all types. Setting `@fallback_to_any true` will explicitly tell the protocol to fallback to `Any` when an implementation for that type cannot be found. The available types are:
